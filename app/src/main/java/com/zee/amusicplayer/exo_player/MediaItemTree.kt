...
object MediaItemTree {
    ...
    private class MediaItemNode(val item: MediaItem) {
        ...
        override fun toString(): String {
            return  "item id:${item.mediaId} title:${item.mediaMetadata.title}"
        }
    }
    ...
    fun initialize(dataSource: SongRepository) {
        ...
        val mediaList = dataSource.songs()
        for (i in mediaList) {
            addNodeToTree(i)
...
//        val genre = mediaObject.getString("genre")
    ...
    fun getChildren(id: String): List<MediaItem>? {
        if (id == Constants.AUDIO_BOOK_ID) {
            return titleMap.values.map { it.item }
        }
